{
	"translatorID": "a3872485-a7af-434e-8fc1-d51f62fef7b3",
	"label": "_beck-online MOD v2025-05-20d",
	...
	"lastUpdated": "2025-05-20 12:09:00"
}
...
function detectWeb(doc, _url) {
	...
	// ## Mann - das findet man am besten in den Zotero-Connector Einstellungen im Browser selbst mit Strg+F
	// dort findet man aber auch direkt die Error mit Zeile lol
	//Zotero.debug("## test log");
	
	// das findet man da aber wohl nicht... ?
	//throw "## test"
...
function scrapeCase(doc, url) {
	...
	// ## Mann -- bei BVerfGE wird das Gericht allein durch den Code oben nicht richtig erkannt
	let container = ZU.xpathText(doc, '//*[@id="toccontent"]/ul/li/a[2]');
	if (container == "BVerfGE") {
		item.court = "BVerfG";
	};
	...
	if (documentClassName == 'ZRSPR') {
		...
		// ## Mann -- BVerfGE wird sonst nicht richtig erkannt
		if (!shortCitation) {
			shortCitation = ZU.xpathText(doc, '//span[@class="bf_selected"]//span[@title]');
		};
...
function finalize(doc, url, item) {
	...
	var perma = ZU.xpathText(doc, '//div[@class="doc-link"]/text()');               // Stand: 09.08.2023
	if (perma) {
